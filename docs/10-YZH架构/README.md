...
>
> ⛔ **双关键字铁律（准则 A · 2026-09-24）**：`Id` **永不**作 WHERE 定位 / 关联 / `Id>0`·`Id==0` 的 add·update 分流 / 存在性判定；业务键 **只用 `Code`**（中间表用 `RoleCode+UserCode` 等）。新增 vs 更新 = `GetByCode` 有无。详见 [01-架构总纲 §2.1](./01-架构总纲.md) · [E201](./08-常见错误与修复.md)。
...
│
│   ── 规范类（续）：专项权威 ── ★
├── 16-操作日志架构设计-V1.md       审计日志的写入 / 查询 / 落库
├── 17-行按钮架构统一实施方案-V1.md  RowButtons 配置与前端渲染
├── 18-YZH架构规则-V1.md            规则总集（铁律汇总入口）
├── 19-前端分层与宿主接入规范-V1.md  core / share / 宿主三层接缝与注入契约
├── 20-前后端契约权威表-V1.md       ★ 字段级唯一权威（逐字段对照 + 例外 E1–E7）
├── 21-项目结构设计规范-V1.md       目录/文件/模块落位
├── 22-接口返回规范-V1.md           ★ 返回语义唯一权威（success/err/HTTP 映射/读取顺序）
└── 23-前后端信封统一改造计划-V1.md ★ 信封统一的执行计划 + TODO + 验收矩阵（22 的实施手册）
```
...
>
> **契约两份权威的分工**（★ 二者必须一致）：
> `20` = **字段叫什么**（大小写、逐字段对照、例外清单） ｜ `22` = **字段什么意思**（成败判据、错误出口、HTTP 映射、前端读取顺序）。
> 改字段看 20，改语义看 22，**先改文档再改代码**。
...
| **接口该返回什么、成功/失败怎么判** | [**22-接口返回规范-V1.md**](./22-接口返回规范-V1.md) ← `success` 唯一判据 / `err` 错误出口 / HTTP 映射 / 前端读取顺序 |
| **字段叫什么、大小写归谁** | [20-前后端契约权威表-V1.md](./20-前后端契约权威表-V1.md) ← 逐字段对照 + 例外 E1–E7 |
| **信封统一怎么改、改到哪了** | [**23-前后端信封统一改造计划-V1.md**](./23-前后端信封统一改造计划-V1.md) ← 阶段 F0/P0–P3 + 决策 D1–D6 + 验收矩阵 F-01~F-14 / P-01~P-13 |
