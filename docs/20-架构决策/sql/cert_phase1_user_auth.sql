...
-- 审核流程相关表
ALTER TABLE `cert_audit_task` ADD COLUMN `OrgCode` VARCHAR(50) DEFAULT NULL COMMENT '所属机构编码' AFTER `actual_end_date`;
...
ALTER TABLE `cert_uploaded_file` ADD COLUMN `OrgCode` VARCHAR(50) DEFAULT NULL COMMENT '所属机构编码' AFTER `file_size`;
...
ALTER TABLE `cert_report_task` ADD COLUMN `OrgCode` VARCHAR(50) DEFAULT NULL COMMENT '所属机构编码' AFTER `due_date`;
...
-- Layer 1: 平台管理层（5个角色）
INSERT INTO `sys_role` (`RoleId`, `RoleName`, `parent_id`, `Enable`, `CreateID`, `CreateDate`, `modify_id`, `modify_time`, `order_no`, `delete_reason`) VALUES
...
INSERT INTO `sys_department` (`id`, `department_name`, `department_code`, `parent_id`, `Enable`, `CreateID`, `CreateDate`, `modify_id`, `modify_time`) VALUES
...
-- 更新超级管理员账号（user_type=1, org_code=NULL 表示平台最高权限）
...
/*
INSERT INTO sys_roleauth (id, role_id, module_id, module_type, permission, enable, create_id, create_time)
SELECT 
    NULL,
    100,
    menu_id,
