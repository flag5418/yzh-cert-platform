...
> **⚠️ 过时标注（2026-08-19）**：本文件中 `LlmExtractSkill` / `ISkillNode` / `SkillBase` 相关设计已过时。信息提取不再依赖 AI 临时提取，走独立文档提取流程。当前 Skill 权威文档：`docs/30-项目规则/Skill清单-V1.md`（V1.1）。
...
> 全部设计图使用 **Mermaid** 语法，与《总体设计-V3.md》（docs/90-归档/旧版本）既定格式保持一致，GitHub / VSCode 可直接渲染。
