 ...
 *
 * 本地缓存设计：
 * - 页面加载时调用 getAllAssociations() 获取全部角色-菜单关联（1 次 API）
...
export class RoleMenuLogic {
  ...
  // ──── 本地缓存：Map<roleCode, Set<menuCode>> ────
  ...
  async initCache(): Promise<void> {
    ...
    try {
      ...
      console.error('[RoleMenuLogic] 加载关联缓存失败:', e)
    }
  }

  /** 从关联数组构建 Map<roleCode, Set<menuCode>> */
  ...
  // ========================================================
  // Badge：角色的已授权菜单数量（从本地缓存计算）
  // ========================================================

  /** 获取角色已授权的菜单数量 */
  getMenuCount(roleCode: string): number {
    return this.associationCache.get(roleCode)?.size ?? 0
  }

  /** 获取角色 badge 文本 */
  getRoleBadge(roleCode: string): string | undefined {
    const count = this.getMenuCount(roleCode)
    return count > 0 ? String(count) : undefined
  }

  // ========================================================
  // 角色选择 → 从本地缓存计算 CheckFlag（无需额外 API）
  // ========================================================
  ...
  async handleRoleSelect(role: RoleTreeItem): Promise<void> {
    ...
    try {
      ...
      // 将 Extra（Url/Icon/Tag 等）平铺到节点，便于表格列直接展示
      for (const node of data) {
        if (node.Extra) Object.assign(node, node.Extra)
      }

      // 用本地缓存覆盖 CheckFlag（不依赖后端返回）
      ...
      ElMessage.error(e.message || '加载菜单数据失败')
  ...
  /** 后端补全祖先后，拉取最新关联，保证 badge 与回显准确 */
  private async syncCacheFromServer(): Promise<void> {
    try {
      const associations = await getAllAssociations()
      this.buildCache(associations)
    } catch (e: any) {
      console.error('[RoleMenuLogic] 同步关联缓存失败:', e)
    }
  }
