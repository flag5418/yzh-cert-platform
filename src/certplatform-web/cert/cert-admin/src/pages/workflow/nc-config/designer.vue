  ...
  <div class="nc-config-page studio-layout">
    <div class="page-header">
      <div class="header-left">
        ...
        <span class="header-title">NC 规则配置</span>
      </div>
    </div>

    <div class="page-body">
      <!-- ===== 左栏：统一4级树（可折叠，修复评审 P0-2 画布塌陷） ===== -->
                      ...
                      <div v-if="phase.visible && phase.expanded && phase.ruleLoaded">
                        ...
                        >
                          <el-icon class="type-icon rule" :class="{ configured: !!rule.ruleJson }">
                            <CircleCheck v-if="rule.ruleJson" />
                            <Document v-else />
                          </el-icon>
      ...
      <main class="main-content">
        <div class="canvas-toolbar">
          <span class="canvas-title">{{
            currentRule ? `工作流：${currentRule.ruleName || currentRule.RuleName || currentRule.ruleCode || currentRule.RuleCode || '未命名'}` : '请选择 NC 检查项'
          }}</span>
...
// 面板折叠状态（修复评审 P0-2：视口窄时画布被挤到 0 宽导致 LogicFlow 不渲染、拖拽失效）
...
// 面板折叠后由 ResizeObserver 自动触发 diagram.resize，无需额外处理
...
async function loadRulesForPhase(phase) {
  ...
  try {
    const filters = []
    if (currentFilter.orgCode) filters.push({ Field: 'OrgCode', Value: currentFilter.orgCode, Operator: 'Equal' })
    if (currentFilter.standardCode) filters.push({ Field: 'StandardCode', Value: currentFilter.standardCode, Operator: 'Equal' })
    if (currentFilter.phaseCode) filters.push({ Field: 'PhaseCode', Value: currentFilter.phaseCode, Operator: 'Equal' })
    const res = await yzhApi.post('/api/ValidationRule/filter', { Page: 1, PageSize: 200, Filters: filters })
    const items = res?.data?.Items || res?.data?.items || res?.data || []
    phase.children = items.map((r) => ({ ...r, id: r.RuleCode || r.Code, ruleName: r.RuleName || r.ruleName }))
...
function initDiagram() {
  // 修复评审 P0-2：容器尺寸为 0 时 LogicFlow 内部报错且不渲染 SVG（网格/节点全部丢失，拖拽自然失效）。
  // 轮询等待容器拿到真实尺寸后再初始化（最多 20×100ms）
  const ensureSizeThenInit = (attempt = 0) => {
    ...
    if (attempt >= 20) {
      console.warn('[designer] 画布容器尺寸异常（宽高 < 50px），LogicFlow 延迟初始化放弃；请检查布局或展开面板')
      createLogicFlowInstance() // 仍然创建，依赖 ResizeObserver 自愈
      return
    }
...
function createLogicFlowInstance() {
  diagram.value = new LogicFlow({
    ...
    // 字符串形式会被 LF 内部当样式对象按字符索引展开（{0:'#',1:'f'...}），
    // 数字索引键进入 Preact style diff 后触发 SVG CSSStyleDeclaration 原生
    // indexed setter 异常，整个画布 SVG 渲染中断（拖拽节点无处渲染）。
  ...
  // 拖拽节点落点无法渲染、画布看起来是空壳（评审 P0-2 的真正根因）。
  try {
  ...
  // 初始化后立即校准一次尺寸（窄视口下 LogicFlow 首帧可能拿到 0 宽）
  nextTick(() => {
    if (diagram.value && canvasRef.value) {
      const { clientWidth, clientHeight } = canvasRef.value
      if (clientWidth > 0 && clientHeight > 0) {
...
function onCanvasDrop(event) {
  try {
    ...
    // 兜底自愈：画布 SVG 未渲染（窄视口塌陷遗留）时，先校准尺寸再重试坐标换算
    if (diagram.value && canvasRef.value) {
      const hasSvg = !!canvasRef.value.querySelector('svg')
      if (!hasSvg) {
        const { clientWidth, clientHeight } = canvasRef.value
    ...
    if (!diagram.value) {
      ElMessage.error('画布尚未初始化完成，请稍后重试')
      return
    }
...
async function selectRule(rule, phase) {
  ...
  try {
    const res = await yzhApi.post('/api/ValidationRule/filter', {
      Page: 1, PageSize: 1,
      Filters: [{ Field: 'RuleCode', Value: rule.RuleCode || rule.ruleCode, Operator: 'Equal' }]
    })
    const items = res?.data?.Items || res?.data?.items || []
    if (items.length > 0) {
      const d = items[0]
      const merged = {
        ...rule,
        ...d,
        id: d.Code || d.Code || rule.id,
        ruleName: d.RuleName || d.ruleName || rule.ruleName,
        ruleCode: d.RuleCode || d.ruleCode || rule.ruleCode,
        ruleJson: d.RuleJson || d.ruleJson,
        layoutJson: d.LayoutJson || d.layoutJson,
        orgCode: d.OrgCode || d.orgCode,
        standardCode: d.StandardCode || d.standardCode,
        phaseCode: d.PhaseCode || d.phaseCode,
        clauseCode: d.ClauseCode || d.clauseCode,
      }
      Object.assign(rule, merged)
      currentRule.value = rule
      renderWorkflow(merged.ruleJson, merged.layoutJson)
    } else {
      clearCanvas()
      ensureStartNode()
    }
...
function renderWorkflow(ruleJson, layoutJson) {
  ...
  try {
    ...
    // 加载到 store（修复评审 P0-3：加载已有规则后立即标记 clean，
    // 避免页脚误报「未保存」、保存按钮误点亮）
...
async function handleSave() {
  ...
  try {
    ...
    const savePayload = {
      Code: currentRule.value.code || currentRule.value.Code,
      OrgCode: currentRule.value.orgCode || currentRule.value.OrgCode || currentFilter.orgCode,
      StandardCode: currentRule.value.standardCode || currentRule.value.StandardCode || currentFilter.standardCode,
      PhaseCode: currentRule.value.phaseCode || currentRule.value.PhaseCode || currentFilter.phaseCode,
      ClauseCode: currentRule.value.clauseCode || currentRule.value.ClauseCode,
      WorkflowCode: currentRule.value.workflowCode || currentRule.value.WorkflowCode,
      RuleCode: currentRule.value.ruleCode || currentRule.value.RuleCode,
      RuleName: currentRule.value.ruleName || currentRule.value.RuleName,
      RuleNameEn: currentRule.value.ruleNameEn || currentRule.value.RuleNameEn,
      SeverityIfViolated: currentRule.value.severityIfViolated || currentRule.value.SeverityIfViolated || 'minor',
      RuleJson: JSON.stringify(config),
      LayoutJson: JSON.stringify(layout),
      NcDescriptionTemplate: currentRule.value.ncDescriptionTemplate || currentRule.value.NcDescriptionTemplate,
      IsActive: currentRule.value.isActive !== false,
      Remark: currentRule.value.remark || currentRule.value.Remark
    }
    const isNew = !savePayload.Code
    const url = isNew ? '/api/ValidationRule/add' : '/api/ValidationRule/update'
    const res = await yzhApi.post(url, savePayload)
...
/* 修复评审 P0-2：画布容器必须有确定尺寸，否则 LogicFlow 初始化失败（不渲染 SVG）且拖拽 drop 无效。
   min-width 保底：即使视口窄（内容区 783px），画布也至少 240px，两侧面板自动收缩 */
.left-panel { width: 280px; min-width: 200px; flex-shrink: 1; flex-basis: 280px; }
.right-panel { width: 360px; min-width: 260px; flex-shrink: 1; flex-basis: 360px; }
...
.right-panel { background: #fff; border-left: 1px solid #e2e8f0; display: flex; flex-direction: column; overflow: hidden; }
