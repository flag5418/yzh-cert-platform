  ...
  <div class="execution-result-panel">
    ...
    <div class="result-body">
        ...
        <el-collapse v-model="activePaths">
          <el-collapse-item v-for="path in result.pathResults" :key="path.pathIndex" :name="path.pathIndex">
            ...
            <div class="path-detail">
              ...
              <div v-if="path.output" class="path-output"><span>输出:</span><pre class="nc-json">{{ formatJson(path.output) }}</pre></div>
            </div>
          </el-collapse-item>
        </el-collapse>
      </div>
      <div class="task-info-section">
        ...
        <div class="task-info"><div><span class="info-label">TaskCode:</span> {{ result.taskCode }}</div><div><span class="info-label">ItemCode:</span> {{ result.itemCode }}</div></div>
      </div>
    </div>
  </div>
</template>
...
.path-header { display: flex; align-items: center; gap: 8px; .path-index { font-weight: 600; } .path-nodes { color: #909399; font-size: 12px; } }
