...
async function handleLogin() {
  ...
  try {
    ...
    // 保存 token 和用户信息（双键设计：Id + Code）
    const { token, userId, userCode, userName, userTrueName, roleId } = res.data
    authStore.setToken(token)
    authStore.setUserInfo({ token, userId, userCode, userName, userTrueName: userTrueName ?? '', roleId: roleId ?? 0 })
