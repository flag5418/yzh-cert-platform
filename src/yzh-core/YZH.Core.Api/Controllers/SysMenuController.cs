...
public class SysMenuController : YzhControllerBase<Sys_Menu>
{
    ...
    protected override async Task OnBeforeAdd(Sys_Menu entity)
    {
        // 校验编码唯一性
        var result = await Entity.ExistsByCodeAsync(entity.Code);
        if (result.Data == true)
        {
            throw new InvalidOperationException($"菜单编码 {entity.Code} 已存在");
        }
    ...
    protected override async Task OnBeforeUpdate(Sys_Menu entity)
    {
        // 校验编码唯一性（排除自身）
        var result = await Entity.ExistsByCodeAsync(entity.Code);
        if (result.Data == true)
