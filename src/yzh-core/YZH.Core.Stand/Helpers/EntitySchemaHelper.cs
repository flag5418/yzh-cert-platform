...
///
///     字段名约定（V4 强制）：
///     - NewEntity/Schema 的 key 必须等于 Columns[].FieldName = C# 实体属性名 = PascalCase
///     - 前端 client.ts 不做 camelCase/PascalCase 互转，原样透传
///     - YZH 架构铁律：JSON 字段名 = 实体属性名 = 数据库列名 = PascalCase
/// </summary>
public static class EntitySchemaHelper
{
  ...
  /// <returns>PascalCase 字段名 → 字段描述</returns>
  ...
  public static Dictionary<string, EntityFieldSchema> GetSchema(Type entityType)
  {
    ...
    foreach (var prop in properties)
    {
      ...
      // V4 强制：保持 PascalCase（与 Columns[].FieldName 一致）
      var fieldName = prop.Name;
      var (clrType, optional) = UnwrapNullable(prop.PropertyType);

      schema[fieldName] = new EntityFieldSchema
      {
  ...
  /// </summary>
  /// <typeparam name="T">实体类型</typeparam>
  /// <returns>PascalCase 字段名 → 默认值</returns>
