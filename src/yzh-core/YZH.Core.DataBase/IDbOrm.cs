...
public interface IDbOrm
{
    ...
    Task<Result<T?>> GetOneAsync<T>(Expression<Func<T, bool>> predicate) where T : class;

    /// <summary>获取列表</summary>
    Task<Result<List<T>>> GetListAsync<T>(Expression<Func<T, bool>>? predicate = null) where T : class;

    /// <summary>分页查询</summary>
    Task<Result<(List<T> items, int total)>> GetPageAsync<T>(SqlPageOptions options) where T : class;

    /// <summary>统计数量</summary>
    Task<Result<int>> CountAsync<T>(Expression<Func<T, bool>>? predicate = null) where T : class;

    /// <summary>判断是否存在</summary>
    Task<Result<bool>> ExistsAsync<T>(Expression<Func<T, bool>> predicate) where T : class;

    // ==================== 写入 ====================

    /// <summary>新增</summary>
    Task<Result<T>> InsertAsync<T>(T entity) where T : class;

    /// <summary>批量新增</summary>
    Task<Result<int>> InsertBatchAsync<T>(IEnumerable<T> entities) where T : class;

    /// <summary>更新（全字段）</summary>
    Task<Result<T>> UpdateAsync<T>(T entity) where T : class;

    /// <summary>更新（指定字段）</summary>
    Task<Result<T>> UpdateAsync<T>(T entity, params string[] fields) where T : class;

    /// <summary>批量更新</summary>
    Task<Result<int>> UpdateBatchAsync<T>(IEnumerable<T> entities) where T : class;

    /// <summary>按 Code 删除</summary>
    Task<Result<bool>> DeleteByCodeAsync<T>(string code) where T : class;

    /// <summary>按 Code 批量删除</summary>
    Task<Result<int>> DeleteByCodeBatchAsync<T>(IEnumerable<string> codes) where T : class;
