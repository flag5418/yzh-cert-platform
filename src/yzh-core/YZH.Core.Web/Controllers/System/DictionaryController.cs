...
[Authorize]
public class DictionaryController : YzhControllerBase<Sys_Dictionary>
{
    ...
    protected override async Task<(bool ok, string? msg)> OnBeforeAdd(Sys_Dictionary entity)
    {
        // 校验编码唯一性
    ...
    protected override async Task<(bool ok, string? msg)> OnBeforeUpdate(Sys_Dictionary entity)
    {
        // 校验编码唯一性（排除自身）
        var result = await Entity.ExistsByCodeAsync(entity.DicNo);
        if (result.Data == true)
