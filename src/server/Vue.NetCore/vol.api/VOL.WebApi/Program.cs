...
// ====== Office 文档转换后台服务 ======
builder.Services.AddScoped<VOL.CERT.Services.Admin.Platform.OfficeConvertService>();
...
builder.Services.AddSingleton<YZH.Core.Queue.IYzhTaskExecutor, VOL.CERT.Services.Admin.Platform.OfficeConvertTaskExecutor>();
// 队列取消后的业务清理钩子：上传任务取消时彻底清理数据库记录 + MinIO 对象
builder.Services.AddScoped<YZH.Core.Queue.IYzhQueueCancelHandler, VOL.CERT.Services.Admin.Platform.UploadQueueCancelHandler>();
builder.Services.AddHostedService<YZH.Core.Queue.YzhQueueHostedService>();

// ====== SignalR 转换进度通知（桥接队列引擎与 Hub） ======
builder.Services.AddScoped<VOL.CERT.IServices.Admin.Platform.IConvertNotifier, VOL.WebApi.Hubs.ConvertNotifier>();
builder.Services.AddSingleton<YZH.Core.Queue.IYzhQueueNotifier, VOL.CERT.Services.Admin.Platform.CertQueueNotifier>();
...
builder.Services.AddScoped<YZH.Core.Workflow.ISkillRegistry, VOL.CERT.Services.Admin.Platform.WorkflowEngine.CertSkillRegistry>();
...
// 工作流执行引擎（V3 模块注册）
builder.Services.AddScoped<VOL.CERT.Services.Admin.Platform.WorkflowEngine.WorkflowConfigParser>();
builder.Services.AddScoped<VOL.CERT.Services.Admin.Platform.WorkflowEngine.NodeExecutor>();
builder.Services.AddScoped<VOL.CERT.Services.Admin.Platform.WorkflowEngine.WorkflowInterpreter>();
builder.Services.AddScoped<VOL.CERT.Services.Admin.Platform.WorkflowEngine.TaskCacheService>();
builder.Services.AddSingleton<VOL.CERT.Services.Admin.Platform.WorkflowEngine.WorkflowLogger>();
builder.Services.AddScoped<VOL.CERT.Services.Admin.Platform.WorkflowEngine.WfExecutionTaskService>();
...
// 新增Helper服务
            ...
            builder.Services.AddScoped<VOL.CERT.IServices.Admin.Platform.IMinIOHelper, VOL.CERT.Services.Admin.Platform.MinIOHelper>();
builder.Services.AddScoped<VOL.CERT.IServices.Admin.Platform.IFolderFileManager, VOL.CERT.Services.Admin.Platform.FolderFileManager>();
builder.Services.AddScoped<VOL.CERT.IServices.Admin.Platform.IFileStorageService, VOL.CERT.Services.Admin.Platform.FileStorageService>();
