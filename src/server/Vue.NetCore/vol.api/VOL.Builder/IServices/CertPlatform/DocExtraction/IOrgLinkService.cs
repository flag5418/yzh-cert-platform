 ...
 * 
 * V3 适配：
 *   - cbCode 参数实际是 CertificationBody.Code（如 CB001-CODE）
 *   - cert_org_stage.org_code 存储 CertificationBody.Code
 *   - cert_org_stage.phase_code 关联 cert_cert_stage.phase_code
 *   - 返回的 ID 列表是 CertStage.Id（前端用 Id 匹配 checkbox）
 */
...
namespace VOL.Builder.IServices.CertPlatform
{
    public interface IOrgLinkService : IDependency
    {
        ...
        /// 同步机构-标准关联
        /// </summary>
        /// <param name="cbCode">机构编码（CertificationBody.Code）</param>
