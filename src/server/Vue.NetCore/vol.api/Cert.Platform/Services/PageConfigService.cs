...
namespace YZH.Core.Services
{
    ...
    ///
    /// 职责：
    /// 1. 从数据库查询页面级配置（PageConfig）
    /// 2. 查询字段级配置（FieldConfig）并组装为 DTO
    ...
    /// </summary>
    public class PageConfigService : IYzhPageConfigService
    {
        private readonly VOLContext _db;

        public PageConfigService(VOLContext db)
        {
            _db = db;
        }
        ...
        public async Task<PageConfigResult> GetPageConfigAsync(string pageKey)
        {
            ...
            var pageMeta = await _db.Set<PageConfig>()
            ...
            var fieldEntities = await _db.Set<FieldConfig>()
        ...
        {
            return await _db.Set<PageConfig>()
        ...
        public async Task<AllConfigsResult> GetAllConfigsFullAsync()
        {
            ...
            {
                // 1. 获取所有活跃页面
                var pages = await _db.Set<PageConfig>()
                    .Where(c => c.IsActive == 1)
                    .ToListAsync();
                ...
                var versionDate = await _db.Set<FieldConfig>()
                ...
                var allFields = await _db.Set<FieldConfig>()
                    .ToListAsync();
        ...
        /// </summary>
        private async Task<PageConfigResult> BuildPageConfig(PageConfig pageMeta, List<FieldConfig> fieldEntities)
        {
        ...
        /// 将数据库实体映射为 DTO（复用方法）
        /// </summary>
        private static FieldConfigDto MapFieldToDto(FieldConfig f) => new()
        {
