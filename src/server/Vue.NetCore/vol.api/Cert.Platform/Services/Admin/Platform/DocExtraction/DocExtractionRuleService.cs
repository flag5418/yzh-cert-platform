...
using YZH.Entity.Admin.Platform.Ent;
...
namespace Cert.Platform.Services.Admin.Platform
{
    ...
    , IDocExtractionRuleService, IDependency
    {
        ...
        private async Task SyncExtractionResultToB08B09Async(CertDocExtractionRule rule, SaveExtractionRuleRequest request)
        {
            ...
            const string yzhStdEnt = YZH.Entity.Admin.Platform.CertPlatformConstants.YZH_STANDARD_ENTERPRISE_CODE;
        ...
        public async Task<List<object>> GetConfiguredRulesAsync()
        {
            ...
            var rules = await (from r in db.Set<CertDocExtractionRule>().Where(x => x.Enable)
                               join f in db.Set<YZH.Entity.Admin.Platform.Dir.StandardDirectoryFile>()
