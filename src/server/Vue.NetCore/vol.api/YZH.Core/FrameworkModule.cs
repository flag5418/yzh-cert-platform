...
// 删除策略（DeleteMode / DeleteStrategyAttribute）已合并至 ServiceBase.cs
...
namespace YZH.Core
{
    ...
    public class FrameworkModule : Autofac.Module
    {
        ...
        private void RegisterBaseInfrastructure(ContainerBuilder builder)
        {
            ...
            // 示例：当 EntityMetadataService 实现后取消注释
            // builder.RegisterType<EntityMetadataService>()
        ...
        {
            // TODO: 注册校验执行器
            // builder.RegisterType<ValidationExecutor>()
        ...
        /// <summary>
        /// 删除策略已合并至 ServiceBase.Del()，通过 [DeleteStrategy] 特性声明。
        /// 此方法保留为空壳，标记"已迁移"。
        /// </summary>
        private void RegisterDeleteStrategyServices(ContainerBuilder builder)
        {
            // 已迁移：删除策略逻辑已融入 ServiceBase.Del()
            // 通过 [DeleteStrategy(Mode=Logical|Physical|Cascade)] 特性声明
        }
