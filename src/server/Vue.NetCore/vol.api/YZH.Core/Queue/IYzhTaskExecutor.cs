...
namespace YZH.Core.Queue
{
    ...
    public interface IYzhTaskExecutor
    {
        ...
        /// </summary>
        Task<TaskExecutionResult> ExecuteAsync(QueueTask task, CancellationToken cancellationToken);

        /// <summary>
        /// 任务状态变更钩子（业务侧联动，如恢复资源可见性、清理临时数据）
        /// newStatus: pending(退避重试) / failed(最终失败) / cancelled(取消)
        /// </summary>
        Task OnTaskStateChangedAsync(QueueTask task, string newStatus, string message);
    }
}
