...
///
///   // 显式物理删除
///   [DeleteStrategy(Mode = DeleteMode.Physical)]
///   public class TempFile : EntityBase { }
///
///   // 级联删除
///   [DeleteStrategy(Mode = DeleteMode.Cascade, CascadeEntities = new[] { typeof(OrderDetail) })]
...
/// - 单表 CRUD → TableServiceBase
/// - 左树右表 → TreeTableServiceBase
/// - 纯树形   → TreeServiceBase
/// - 关联表   → LinkTableServiceBase
/// </summary>
public abstract class ServiceBase<TEntity, TRepository>
    : BaseProvider.ServiceBase<TEntity, TRepository>
    ...
    where TRepository : IRepository<TEntity>
{
    ...
    /// <summary>
    /// 逻辑删除实现。
    /// 默认：EntityBase 派生实体走 YZH 统一软删语义（Enable=false + 删除审计字段），
    ...
    protected virtual WebResponseContent ExecuteLogicalDelete(object[] keys, List<TEntity> entities)
    {
        ...
        if (entities[0] is not EntityBase)
    ...
    public override WebResponseContent Del(object[] keys, bool delList = true)
    {
        ...
        var deleteAttr = typeof(TEntity).GetCustomAttribute<DeleteStrategyAttribute>();
    ...
    protected virtual WebResponseContent ExecuteCascadeDelete(
        object[] keys, List<TEntity> entities, DeleteStrategyAttribute? attr)
    ...
    {
        if (entity is not EntityBase yzhEntity) return;
