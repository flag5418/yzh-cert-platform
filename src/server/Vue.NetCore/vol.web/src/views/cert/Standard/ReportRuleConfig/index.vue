<template>
  <div class="report-rule-page studio-layout">
                  ...
                  <template v-if="std.expanded && std.children && std.visible">
                    <template v-for="phase in std.children" :key="phase.id">
                      ...
                      <div v-if="phase.visible && phase.expanded && phase.sectionLoaded">
                        ...
                        >
                          <el-icon
                            class="type-icon rule"
                            :class="{ configured: !!section.workflowConfig }"
                          >
      ...
      <main class="main-content">
        <div class="canvas-toolbar">
          <span class="canvas-title">{{
            currentSection ? `工作流：${currentSection.sectionName}` : '请选择报告章节'
          }}</span>
...
async function loadSectionsForPhase(phase) {
  ...
  try {
    ...
    const res = await proxy.http.get(
      `api/report-definition/section/by-context?${params}`,
      null,
      false
    )
    if (res?.status) {
