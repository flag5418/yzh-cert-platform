...
const loadStageFiles = async (stageNode) => {
  ...
  console.log('📡 stageNode:', JSON.stringify({ id: stageNode.id, name: stageNode.name, directoryCode: stageNode.directoryCode }));
...
const transformFolderTree = (nodes, depth = 0) => {
  return (nodes || []).map((node, index) => {
    ...
    if (isFile) {
      ...
      id = node.FolderCode || node.folderCode || node.Code || node.code || `folder-${depth}-${index}`;
    }
