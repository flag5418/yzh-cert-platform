<template>
  ...
  >
    ...
    >
      ...
      <el-table-column
        ...
        :show-overflow-tooltip="
          ($global.table.showOverflowTooltip && column.showOverflowTooltip !== false) ||
        ...
        <template #default="scope">
          ...
          >
            <template #default="scopeChildren">
              ...
              <div
                v-else-if="
                  columnChildren.formatter && typeof columnChildren.formatter === 'function'
                "
                @click="
            ...
            <div @click.stop class="e-item">
             ...
             如果使用的是date/datetime类型,需要设置表单配置的字段属性edit.valueFormat='YYYY-MM-DD HH:mm' -->
                ...
                <el-tree-select
                  ...
                  :show-checkbox="true"
                  :check-strictly="column.checkCtrictly === undefined ? true : column.checkCtrictly"
                  check-on-click-node
                  node-key="key"
                  @change="column.onChange && column.onChange(scope.row, column)"
                  :props="{ label: 'label' }"
    ...
    <template v-if="!paginationHide">
      <div class="block pagination" :class="[tableV2 ? 'v2-pagination' : '']" key="pagination-01">
        <div>
...
import { getDateFormat, getDateOptions } from './VolTable/VolTableDate.js'
import { initDataSource } from './VolTable/VolTableDicData.js'
import { rowEndEdit, tableRowClick, tableValidate } from './VolTable/VolTableEdit.js'
import {
  expandTreeChange,
  extraClick,
  inputChange,
...
import { getNextTableCell, regTableEventNext } from './VolTable/VolTableEventNext.js'
import { getPath, previewImg } from './VolTable/VolTableFile.js'
import { cellFormatter, formatDate, getCellColor } from './VolTable/VolTableFormat.js'
import { loadData } from './VolTable/VolTableLoadData.js'
...
// 背景颜色、合计
const initConfig = () => {
  initCellStyleSummary(props, proxy, cellStyleColumns, summaryData, summaryIndex, summaryDataV2)
}
...
const dowloadFile = (file, column, index, row) => {
  //file,column,fIndex
  if (column && column.fileClick && column.fileClick(index, file, [file], row) === false) {
    return
  }
