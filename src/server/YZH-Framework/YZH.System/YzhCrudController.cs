...
namespace YZH.System
{
    ...
    /// 暴露与 YZHBaseApiClient 兼容的信封：GetPageData / Add / Update / Del。
    /// </summary>
    public abstract class YzhCrudController<T> : ControllerBase where T : class
    {
        protected readonly YzhService<T> _svc;

        protected YzhCrudController(YzhService<T> svc)
        {
            _svc = svc;
        }
