...
using Phase = SharedEntities::YZH.Entity.Admin.Platform.Cert.PhaseDefinition;
using Link = SharedEntities::YZH.Entity.Admin.Platform.Sys.CertOrgStage;
    ...
    public async Task<ActionResult<ApiResponse<object?>>> Save([FromBody] SaveRequest request)
    {
        try
        {
            if (string.IsNullOrEmpty(request.OrgCode) || string.IsNullOrEmpty(request.StageCode))
                return BadRequest(ApiResponse.Fail("OrgCode 和 StageCode 不能为空"));
