...
namespace CertPlatform.Admin.Services.Workflow
{
    ...
    /// <para>移植自：旧 WorkflowConfigParser.cs（341 行，逻辑零改动，仅 namespace 调整）</para>
    /// <para>职责：</para>
    ...
    public class WorkflowConfigParser
    {
        ...
        /// <summary>路径枚举节点数上限（防御复杂图路径爆炸，迁移计划 R-5）</summary>
        private const int MaxTotalPathNodes = 5000;
        ...
        public ParsedWorkflow Parse(string ruleJson)
        {
            ...
            if (config.Nodes.Count > 100)
                throw new InvalidOperationException($"节点数超限：{config.Nodes.Count} > 100（防御性上限）");
            ...
            Dictionary<string, WorkflowNodeConfig> nodeMap)
        {
            ...
            var totalNodes = paths.Sum(p => p.Count);
            if (totalNodes > MaxTotalPathNodes)
                throw new InvalidOperationException($"路径枚举节点总数超限：{totalNodes} > {MaxTotalPathNodes}，工作流配置可能过于复杂");
