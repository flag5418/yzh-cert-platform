  ...
  >
    <template #gridHeader>
      ...
      <div class="phase-tabs">
        <el-radio-group v-model="selectedPhase" size="small" @change="filterByPhase">
    ...
    <template #btnLeft>
      <el-button
        ...
        type="primary"
        @click="assignAuditor"
        :disabled="!selectedRow || selectedRow.status !== 'pending_assignment'"
      >
        分配审核员
      </el-button>
      <el-button
...
const filterByPhase = (value) => {
  if (value) {
    // 添加阶段筛选条件
    gridRef.searchFormFields.phase_type = value;
  } else {
    delete gridRef.searchFormFields.phase_type;
  }
  gridRef.refresh();
};
...
const startTask = () => {
  ...
  proxy.$confirm(
    `确定要开始任务 ${selectedRow.value.task_number} 吗？`,
    '确认开始',
...
const completeTask = () => {
  ...
  proxy.$confirm(
    `确定要完成任务 ${selectedRow.value.task_number} 吗？`,
    '确认完成',
...
const viewChecklist = () => {
  ...
  router.push({
    ...
    query: { task_code: selectedRow.value.code },
  });
};
</script>
