...
--   - wf_skill_api (9列)
--   - wf_skill_input (5列)
--   - yzh_queue (3列: id/code/status)
--   - yzh_queue_task (3列: id/code/status)
--   - yzh_queue_resource_lock (1列: create_time 与 CreateTime 重复，DROP)
...
-- cert_ai_usage_log: AI 调用日志
ALTER TABLE cert_ai_usage_log
  CHANGE COLUMN prompt_tokens PromptTokens int NULL DEFAULT 0,
  CHANGE COLUMN completion_tokens CompletionTokens int NULL DEFAULT 0,
  CHANGE COLUMN total_tokens TotalTokens int NULL DEFAULT 0,
  CHANGE COLUMN cost_usd CostUsd decimal(10,6) NULL DEFAULT 0.000000,
  CHANGE COLUMN duration_ms DurationMs bigint NULL DEFAULT 0;

-- wf_skill_api: Skill API 配置
-- 先 DROP 外键（collation 不一致会导致 ALTER 失败），改名后重建
...
ALTER TABLE wf_skill_api
  ...
  CHANGE COLUMN enable Enable tinyint(1) NOT NULL DEFAULT 1,
  CHANGE COLUMN status Status varchar(50) NULL DEFAULT 'active';
-- 重建外键
...
-- yzh_queue: 队列主表（之前遗漏 id/code/status）
ALTER TABLE yzh_queue
  CHANGE COLUMN id Id bigint NOT NULL AUTO_INCREMENT,
  CHANGE COLUMN code Code varchar(36) NOT NULL,
  CHANGE COLUMN status Status varchar(20) NOT NULL DEFAULT 'pending';

-- yzh_queue_task: 队列任务表（之前遗漏 id/code/status）
ALTER TABLE yzh_queue_task
  CHANGE COLUMN id Id bigint NOT NULL AUTO_INCREMENT,
  CHANGE COLUMN code Code varchar(36) NOT NULL,
  CHANGE COLUMN status Status varchar(20) NOT NULL DEFAULT 'pending';

-- yzh_queue_resource_lock: 队列资源锁（create_time 与 CreateTime 重复，直接 DROP）
-- 说明：yzh_queue_resource_lock 已有 CreateTime 列，create_time 为历史遗留冗余列
...
-- ===================================================================
-- 第二部分：Vol 框架内置表（谨慎执行）
-- 影响：YZH.Core.Web 中的 AuthController/MenuController/Roles 等需同步修改
-- ===================================================================
