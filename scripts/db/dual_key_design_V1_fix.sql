-- ============================================================
-- 双键设计 V1 补充脚本（字段已存在时跳过）
-- ============================================================

-- --------------------------------------------------------
-- 1. 回填 Sys_User Code（如果为空）
-- --------------------------------------------------------
UPDATE sys_user SET Code = CONCAT('USER_', LPAD(User_Id, 6, '0')) WHERE Code IS NULL;

-- --------------------------------------------------------
-- 2. 回填 Sys_Role Code（如果为空）+ 语义化编码
-- --------------------------------------------------------
UPDATE Sys_Role SET Code = CONCAT('ROLE_', LPAD(Role_Id, 6, '0')) WHERE Code IS NULL;
UPDATE Sys_Role SET Code = 'ROLE_SUPER_ADMIN' WHERE Role_Id = 1 AND Code = 'ROLE_000001';
...
UPDATE Sys_Menu SET Code = 'MENU_USER_MGMT', Tag = 'admin' WHERE Menu_Id = 9;
UPDATE Sys_Menu SET Code = 'MENU_USER_MGMT_DISABLE', Tag = 'admin' WHERE Menu_Id = 2;
...
-- 回填 Code 数据
UPDATE Sys_RoleAuth ra JOIN Sys_Menu m ON ra.Menu_Id = m.Menu_Id SET ra.MenuCode = m.Code;
UPDATE Sys_RoleAuth ra JOIN Sys_Role r ON ra.Role_Id = r.Role_Id SET ra.RoleCode = r.Code;
UPDATE Sys_RoleAuth ra JOIN sys_user u ON ra.User_Id = u.User_Id SET ra.UserCode = u.Code;
