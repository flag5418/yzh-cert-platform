...
def process_file(file_path):
    ...
    for old, new in mapping.items():
        # Match .status, .code etc
        content = re.sub(rf'\.({old})\b', f'.{new}', content)
        # Match field: 'status' or name: 'status' or query: { task_code: ... }
        content = re.sub(rf"(['\"]){old}\1", f"\\1{new}\\1", content)
        # Match task_code: value in object literal
...
print("Vue and JS files in cert directory updated to PascalCase (v2).")
